...
class GlobalSearch(BaseSearch):
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        # print("=============================")
        # print(f"context_chunks: {len(context_chunks)}")
        ...
        **llm_kwargs,
        ...
        try:
            ...
            try:
                ...
                # print("search_messages:", search_messages)
    ...
    def extract_json_from_response(self, response: str) -> str:
        ...
        try:
            # Use a regex to find the JSON object within the response string
            json_match = re.search(r'(\{.*\})', response, re.DOTALL)
            if json_match:
                return json_match.group(1)
            else:
                log.error("No JSON object found in the response.")
                return ""
    ...
    def parse_search_response(self, search_response: str) -> list[dict[str, Any]]:
        ...
        """
        # Chuck
        json_response = self.extract_json_from_response(search_response)
        if not json_response:
            raise ValueError("Failed to extract JSON from search response.")
        
        parsed_elements = json.loads(json_response)["points"]
